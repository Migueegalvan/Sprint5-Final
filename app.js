...
import { fetchAndStoreCountries } from './services/countryService.mjs'; // Asegúrate de importar la función
...
connect().then(() => {
    // Llamamos a la función que consume la API y guarda los países
    fetchAndStoreCountries().catch(error => {
        console.error('Error al almacenar países desde la API:', error);
    });
});
